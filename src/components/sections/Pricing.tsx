    ...
    return (
        <div id='pricing' className='py-6 flex flex-col items-center sm:py-10'>
            ...
            <div className="grid grid-cols-1 w-4/5 mx-auto  gap-5 md:grid-cols-2 lg:grid-cols-3">
                {Prices.map((price) => (
                    <div key={price.id} className={`border cursor-pointer border-secondary duration-300 hover:shadow-[4px_4px_#000] p-5  h-[400px] flex justify-between  flex-col rounded-lg ${price.name === "Pro" && "scale-105 shadow-[3px_3px_#769ed0] hover:shadow-[4px_4px_#769ed0] "} `}>
                        ...
                        <button className={`bg-primary font-semibold font-dm_sans w-full hover:shadow-[4px_4px_#000] duration-500 py-2 rounded-lg ${price.name === "Pro" && "bg-secondary "}`}>{price.buttonText}</button>
                    </div>
                ))
                }
            </div>
